...
# 15-4. Modified Random Walks: In the class RandomWalk, x_step and y_step are generated from the
# same set of conditions . The direction is chosen randomly from the
# list [1, -1] and the distance from the list [0, 1, 2, 3, 4] .
#  Modify the values in these lists to see what happens to the overall
# shape of your walks . Try a longer list of choices for the distance, such
...
#15-5 The method fill_walk() is lengthy. Create a new method called get_step() to determine
...
# 15-6 Modify die.py and dice_visual.py by replacing the list we used to set the value of hist.
# x_labels with a loop to generate this list automatically.
# If you are comfortable with list comprehensions, try replacing the
...
#15-7 Create a simulation showing what happens if you roll two eight-sided dice 1000 times. Increase the number
# of rolls gradually until you start to see the limits of your systems capabilities.
import pygal
...
#15-8 If you roll three D6 dice, the smallest number you can roll is 3 and the largest number is 18.
...
#15-9 When you roll two dice, you usually add the two numbers together to get the result.
...
plt.scatter(rw.x_values, rw.y_values, s=11)
plt.show()
